...
allowed-tools: Bash(ls *), Bash(bd *), Bash(mv *), Bash(python *), Read, Write, Glob, Grep, Task
---

# Vision

Take a raw vision — spoken, typed, or queued — and turn it into a clear, grounded document. The core job is reading user intent: sometimes that means light polish, sometimes it means helping them think.
...
1. **Check queue:**
   ...
   ```
   If empty or only `.gitkeep`: report "No visions in queue. Record one with: `python .aura/scripts/record_memo.py`" and stop.

2. **For each item**, process sequentially:
   - **Text file** (`.txt`): Read it directly
   - **Audio directory** (has `audio.wav`): Check for `transcript.txt`, transcribe if missing:
     ```bash
...
2. **Do the work:**
   ...
   - **Polish tasks:** Fix typos, remove verbal stuttering (ums, repeated phrases, false starts), tighten language. Preserve the user's voice — don't rewrite their ideas into corporate speak.
